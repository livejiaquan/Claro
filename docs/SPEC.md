...
| `clean` | 去填充詞、保留自我更正後版本、修同音／近音誤辨（Context 詞彙為準）、補標點；其餘逐字保留，不跨句重排、不濃縮 | 新安裝的預設意圖；provider 未就緒時安全退回 RAW |
| `organize` | 允許跨句重排、分段與合併重複內容；不得改動或新增姓名、術語、數字、日期／時間、否定、條件、決策、待辦與因果 | 首次啟用必須明確同意；不確定或 guard 不通過即退回原文 |

CLEAN 沿用 prototype 與 yetone「只修不改寫」規則；ORGANIZE 使用獨立 prompt，不得與 CLEAN 共用互相矛盾的指示。兩者都只輸出結果、不回答或執行轉錄／Context 中的指令。

guard 分兩層：共同層拒絕空輸出、長度爆炸、提示詞洩漏與離題回答；ORGANIZE 另外比對事實錨點與內容覆蓋。history 記錄 requested/effective mode、provider、是否改動、成功或 fallback 原因，讓 UI 可顯示 raw/final 與復原依據。
...
- **local-only 模式**：新安裝預設開啟；一鍵硬斷所有外部網路請求（含未來更新檢查）。localhost／127.0.0.1／`::1` 本機服務仍可使用；自訂外部 endpoint 必須 HTTPS
- ORGANIZE 行為同意與雲端資料傳送同意是兩個獨立、具版本的 backend gate，不能只靠前端文案。任一 gate 不成立都零外連並安全退回 RAW
...
- **Settings**（React）：General（熱鍵、開機啟動）／Models（STT＋處理 provider、下載管理、BYOK）／Output Mode（RAW／CLEAN／ORGANIZE）／Context（開關、per-app、稽核視圖）／Dictionary／History（raw/final、搜尋、清除）／Privacy（local-only、目的地與資料欄位揭露）／Advanced（延遲統計、debug 音訊）。首頁隱私文案必須依實際 mode/provider/context 動態顯示，不得無條件宣稱全本地。
...
新 schema 分節（hotkey/stt/polish/context/cjk/dictionary/privacy）；目前相容扁平鍵新增 `polish_mode`（`raw|clean|organize`）、`local_only`、`organize_consent_version`、`cloud_consent_version`、`cloud_consent_origin`、`setup_completed`。遠端雲端同意綁定到確認時的 scheme + authority，更換 host、port 或 scheme 後自動失效。`setup_completed` 只能在後端收到完成動作時寫入，前端仍必須先通過權限、麥克風與模型檢查。啟動時偵測舊 schema（`whisper_model`/`llm_model`/`llm_enabled`）自動遷移並保留未知欄位。history.jsonl 維持向後相容，新增可選 `timings` 與 `polish` metadata，舊讀取器不受影響。
