...
版本：Phase 0（2026-07-05），P0 Trust & Release Pass 更新（2026-07-12）。本文件是實作的真值來源；重大變更需先更新此文件。
...
1. 熱鍵按下即建立 session；先以最長 40ms 的單一 AX 呼叫固定 focused element reference（不讀文字／metadata），接著開啟麥克風，再完成有界 target hash 與背景 Context 擷取；熱鍵放開/停止 → 收集音訊（防呆：<0.3s、RMS 靜音 → error 態）
...
推理 crate（依 Handy 實證定案，見 `docs/research/handy-analysis.md`）：Whisper/GGUF 家族用 `transcribe-cpp`（Metal）；ONNX 家族（Parakeet/Moonshine/SenseVoice）用 `transcribe-rs`。模型依架構與實體 RAM 推薦：8–12GB／Intel 優先 large-v3-turbo Q5，16GB+ Apple Silicon 優先 large-v3-turbo；低記憶體路徑在 STT 完成後先卸載語音模型，再載入內建 LLM，潤飾後立即卸載 LLM，避免雙模型同時駐留與長時間佔用約 4B 模型記憶體。
...
1. **解碼期偏置**：抽英文／技術詞彙與 CJK 詞彙進 Whisper `initial_prompt`（上限 15 詞）
2. **處理參考**：ScreenContext 詞彙在 STT 解碼期做偏置；CLEAN 不把畫面文字交給 LLM，也禁止 LLM 修改任何文字／英數 token。完整 bounded ScreenContext 只在 ORGANIZE 進 Polisher prompt；可解釋的 surface 分類（email／message／document／technical／neutral）只能決定段落或明確清單格式，不允許改動事實或語氣內容
...
新 schema 分節（hotkey/stt/polish/context/cjk/dictionary/privacy）；目前相容扁平鍵新增 `polish_mode`（`raw|clean|organize`）、`local_only`、`history_enabled`、`organize_consent_version`、`cloud_consent_version`、`cloud_consent_origin`、`setup_completed`。遠端雲端同意綁定到確認時的 scheme + authority，更換 host、port 或 scheme 後自動失效。新安裝的 `setup_completed` 只能在後端同時確認權限、麥克風測試、已驗證模型與本次啟動至少一次成功貼上後寫入；既有已完成設定不被回溯阻擋。啟動時偵測舊 schema（`whisper_model`/`llm_model`/`llm_enabled`）自動遷移並保留未知欄位。history.jsonl 維持向後相容，新增可選 `timings` 與 `polish` metadata，舊讀取器不受影響。
...
| D13 | 首次設定與模型生命週期由唯讀 `HardwareProfile` 決策：依 Apple Silicon／Intel、實體 RAM 與 Apple Intelligence 可用性推薦 STT／LLM；8–12GB 路徑不讓 STT 與 4B LLM 同時常駐。只改推薦與資源策略，任何模型下載仍須使用者明確按下 | **已定並實作（2026-07-12，P0 Product Pass）** |
