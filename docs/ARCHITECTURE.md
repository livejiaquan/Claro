...
- **視窗拖曳（連踩兩坑，使用者實測才暴露）**：`-webkit-app-region` 是 Electron 的，
  ...
  拖曳 IPC 被默默拒絕——capabilities 必須明確加 `core:window:allow-start-dragging`；
  (2) Tauri 內建的 drag-region script 只認 `event.target`「本身」有該屬性，
  子元素（logo、標題文字）全不觸發。因此 `App.tsx` 自己監聽 mousedown 用
  `closest('[data-tauri-drag-region]')` 判定並呼叫 `startDragging()`，
  互動元件（button/input/.no-drag）排除；屬性本身保留當標記。
...
- Swift 膠囊／menu bar（`mic_indicator`）以 **Tauri sidecar** 打進
  `Contents/MacOS/`——release runtime 不依賴 repo 路徑；`npm run build` 會先跑
  `scripts/build-indicator.sh`（需要 rustc/swiftc 在 PATH）。CI 以 bundle contract
  斷言 sidecar 存在、最低系統版本與 runtime 依賴白名單。
- 模型檔完整性：下載固定 immutable revision＋SHA-256，成功後寫
  `<模型>.claro-verified.json` 持久 marker；**沒 marker 的既有檔第一次 verify 要
  整檔 hash（2.5GB 秒級）**——啟動時背景預校驗補 marker，別讓聽寫路徑扛。
...
- 輔助使用權限跟著簽章走：**未簽章的開發版每次重建 cdhash 都變**，權限即失效，
  且**系統設定裡的舊條目看起來仍勾選、重新勾選也救不回**（TCC 綁的是舊簽章）。
