...
- **潤飾層（2026-07-06，SPEC D3 定案）**：providers＝off（預設）/ **Apple Intelligence**（macOS 26+：`swift/apple_intelligence.swift` 靜態連進 binary，build.rs 弱連結、SDK 過舊自動編 stub；**用 DynamicGenerationSchema 不用 @Generable**——CommandLineTools 沒有 macro plugin，且 guided generation 實測是防「把聽寫當指令回答」的關鍵）/ **內建模型**（`llm.rs`：llama-cpp-2/Metal in-process，Qwen3-4B＋Gemma3-4B 目錄制下載，閒置 5 分鐘卸載；與 whisper.cpp 兩份 ggml 共存實測 OK；**CJK 一字多 token——生成的 token 必須整串收完再 detokenize，逐 token 轉會丟字**）/ Ollama / LM Studio（模型清單自動偵測）/ 自訂 API（六組雲端 preset，key 進鑰匙圈）。驗證：`cargo run --example apple_polish`、`cargo run --example builtin_polish [x.wav]`（whisper＋llama 同程序）、`cargo run --example test_polish`（mock server）。
- **上下文與字典（2026-07-06，M3 核心前移）**：`context.rs` AX 擷取（**必須走 NSWorkspace 拿 pid——system-wide AXFocusedApplication 回 -25204**；AXSecureTextField 永不讀；預算 500/1200/400）；詞彙進 initial_prompt、全文進潤飾 prompt；`context_enabled` 開關。個人字典存 config `dictionary` 鍵（設定頁 CRUD，Swift menu bar 會保留未知鍵）。驗證：`cargo run --example show_context`。**驗證鐵律：絕不出聲（不 afplay、不合成全域鍵盤事件）——用 `say -o`＋`transcribe_file`／mock／happy-dom。**
