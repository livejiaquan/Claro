...
- **Codex 專業拼法（2026-07-23 實驗；2026-07-26 深度 review）**：可選 cloud provider，重用已安裝／登入的 Codex CLI，但不讀 credential、不自動安裝或登入。CORRECT 目前只支援 Codex，Codex 除 RAW 暫停外也只支援 CORRECT；切離 Codex 時後端會把 CORRECT 原子降為 CLEAN，任何不相容組合都 fail closed。只在明確同意的 CORRECT 模式送限定文字；主同意以 contract-v2 綁 auth kind、raw／normalized CLI 版本、實際 executable path＋SHA-256、全部動態停用的 feature capability fingerprint 與固定 runner contract，執行前再完整比對。三類候選詞共用 32 項上限，只有與本次轉錄相關且 correction guard 可採用的項目才送；若三類皆空則不啟動 Codex、不用模型額度，history 以 `codex_payload_started=false` 明確稽核。此欄位只有在至少一個轉錄 payload byte 寫入主 `codex exec` 後才為 true，不能拿「進入 runner」誤當作已外送。畫面候選還需另行同意，raw App 名／視窗標題、含數字詞、無關項目、完整 Context 與音訊不送。深度 review 已證實 target-only separator matching 無法區分 `G PT→GPT`／`A PI→API` 與 `Py Torch→PyTorch`／`The Rapist→TheRapist`，因此自動空白合併全面關閉，已知 source→target 一律交給個人字典；Codex 目前只保留尾端兩字母的極窄單連字號 heuristic（如 `Clau-de→Claude`），**仍不是語意證明**，正式推薦前必須通過真人 false-correction gate。自我測試取消會中止 stdin writer 與 runtime preflight，只作用於該 request；capability probe 使用隔離 HOME＋cwd，退出時會回收正式執行與 probes；所有失敗仍退 deterministic base text。
