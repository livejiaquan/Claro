...
def test_hotkey_handlers_drive_recording_handsfree_processing_and_esc(monkeypatch):
    ...
    monkeypatch.setattr(main, "_do_stop_and_process", lambda *a: calls.append("process"))
...
def test_stop_and_process_pastes_success_and_records_history(monkeypatch):
    ...
    monkeypatch.setattr(main, "_transcribe", lambda *a: "GBT 測試")
    monkeypatch.setattr(main, "_get_window_context", lambda: "")
    ...
    main._models_ready.set()

    main._do_stop_and_process(threading.Event(), main._sm.session)
